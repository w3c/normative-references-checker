...
const view = require('./views/default.js');
...
function getNormativeHref(document, url) {
  ...
  anchors.forEach(anchor => {
    ...
    if (!href.startsWith(docURL) && !references.isInformative(anchor) && !references.isInInformative(anchor)) {
      if (references.isNormativeReference(anchor)) {
        if (!references.isInNormativeReferenceSection(anchor)) {
...
Links.getLinks = function(document, url) {
  references.init(document);
  return getNormativeHref(document, url);
};
